#!/bin/bash
#SBATCH -J allalgos_crash_confirm
#SBATCH -p Physics
#SBATCH -N 1
#SBATCH -n 1
...
RUN_TAG="${RUN_TAG:-eval_allalgos_crash_aware_portfolio_confirm_100ep_20260702}"
...
case "$ALGO" in
  td3) CANDIDATE="nocicim_soft_front_trim" ;;
  epo) CANDIDATE="nocicim_crash_aware_route" ;;
  rec) CANDIDATE="nocicim_route_preserve" ;;
  qpsl) CANDIDATE="nocicim_soft_crash_guard" ;;
  lag) CANDIDATE="nocicim_close_range" ;;
  fac) CANDIDATE="nocicim_monitor_only" ;;
  *) echo "unsupported algo: $ALGO" >&2; exit 2 ;;
esac

echo "Running crash-aware confirmation: tag=${RUN_TAG} algo=${ALGO} train_seed=${SEED} candidate=${CANDIDATE} episodes=${EPISODES}"
